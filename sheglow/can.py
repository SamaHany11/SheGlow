...
app.config['SQLALCHEMY_DATABASE_URI'] =('mssql+pyodbc://@DESKTOP-G7OG4EV/project?'
...
def home():
    ...
    products = Product.query.limit(4).all()  # Fetch first 3 products
...
def products_by_category(product_cat):
    sort_by = request.args.get('sort_by', 'ProductName')  # Default sort by ProductName
...
def check_session_expiration():
    # Only check session expiration if the user is not already on the login page
    if 'login_time' in session and request.endpoint != 'login':
        # Ensure that both datetime objects are naive
        login_time = session['login_time']

        if login_time.tzinfo is not None:  # If session login time is timezone-aware
            login_time = login_time.replace(tzinfo=None)  # Make it naive

        if datetime.now() - login_time > timedelta(days=7):
            session.clear()
            flash('Session expired. Please log in again.', 'error')  
            return redirect(url_for('login')) 
    # Continue to the requested route if the session is valid
    return None
...
def login():
    ...
    if request.method == 'POST':
        ...
        # Fetch the client from the database based email
        client = Client.query.filter_by(email=email).first()

        # Check if the client exists and the password matches
...
def create():
    ...
    if form.validate_on_submit():
        ...
        # Create the new client after successful validation
        new_client = Client(
            first_name=form.first_name.data,
            second_name=form.second_name.data,
            email=form.email.data,
            password=form.password.data,  # Store the password as plain text (or hash it)
...
def show_products():
    ...
    else:  # Default to sorting by ProductName
        products = Product.query.order_by(Product.ProductName.asc() if order == 'asc' else Product.ProductName.desc()).all()

    client_id = session.get('client_id')  # Assuming user login is implemented
    wishlist = set()  # Empty set for guests
    if client_id:
...
def product_details(product_id):
    ...
    # استعلام المنتجات المشاهدة للعميل
    ...
    # إضافة المنتج الجديد إلى القائمة دائمًا
    new_recently_viewed = RecentlyViewed(client_id=client_id, product_id=product_id)
    db.session.add(new_recently_viewed)
    db.session.commit()  # تأكيد الإضافة

    # تحميل المنتجات التي تمت مشاهدتها مؤخراً من قاعدة البيانات
...
def search():
    query = request.args.get('q', '')  # Retrieve the search query from the URL
    if query:

        results = Product.query.filter(Product.ProductName.ilike(f'%{query}%')).all()
    else:
        results = []  # No results if there's no query
...
def search_suggestions():
    ...
    if query:
        # Search for products by name and return only the names as suggestions
...
def track_order(order_id):
    if 'client_id' not in session:
        return jsonify({"message": "Please log in to track orders"}), 401

    client_id = session['client_id']
    order = Order.query.get(order_id)

    if not order or order.client_id != client_id:
        return jsonify({"message": "Order not found or unauthorized access"}), 403
...
def request_return_or_exchange():
    try:
        ...
        msg['To'] = email
        msg['Subject'] = subject
        msg.attach(MIMEText(body, 'html'))  # تحديد أن الرسالة بصيغة HTML
...
def add_to_wishlist(product_id):
    if 'client_id' not in session:
        return jsonify({'status': 'error', 'message': 'Please log in to add items to your wishlist.'}), 401
    ...
    if existing_item:
        return jsonify({'status': 'info', 'message': 'This product is already in your wishlist.'})
...
def remove_from_wishlist(product_id):
    ...
    client_id = session['client_id']
    wishlist_item = Wishlist.query.filter_by(client_id=client_id, product_id=product_id).first()
    ...
    if wishlist_item:
        ...
        return jsonify({'status': 'error', 'message': 'Product not found in your wishlist.'})
...
def wishlist():
    ...
    if request.method == 'POST':
        ...
        if not Wishlist.query.filter_by(client_id=client_id, product_id=product_id).first():
            new_wishlist_item = Wishlist(client_id=client_id, product_id=product_id)
...
def dashboard():
    ...
    # Get the order history for the logged-in client
    ...
    for order in orders:
        ...
        for detail in order_details:
            product = Product.query.get(detail.product_id)  # Assuming Product model has product_id as primary key
            products.append({
                'name': product.ProductName,
                'quantity': detail.product_Quantity,
                'price': detail.Price,
                'image_url': product.PhotoPath  # Use PhotoPath for the image URL
            })
...
def add_to_cart_page():
    ...
    for cart_item in cart_items:
        product = cart_item.product  # Accessing the related Product from the CartItem
...
def clear_cart():
    ...
    client_id = session['client_id']
    # Delete all cart items for the client
    CartItem.query.filter_by(client_id=client_id).delete()
    db.session.commit()
